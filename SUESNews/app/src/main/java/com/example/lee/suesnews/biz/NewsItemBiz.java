...
//        }
        ...
        Element itemTable = document.getElementsByClass("page_right").get(0);
        Elements items = document.getElementsByClass("blog_list");
        //Elements items = itemTable.child(0).children();
        for (int i=0;i<items.size();i++){
            newsItem = new NewsItem();
            newsItem.setType(newsType);

          //  Element columnTable = items.get(i).getElementsByClass("blog_list").get(0);
            Element link = null;
            if (newsType == NewsTypes.CSND_TPYE_ALL) {
                 link = items.get(i).getElementsByTag("a").get(1);
            }else {
                 link = items.get(i).getElementsByTag("a").get(0);
            }
            Element imgLink = items.get(i).getElementsByTag("img").get(0);
            String contentUrl = link.attr("href");  //新闻内容链接
            String imageUrl = imgLink.attr("src");  //新闻头像url
            newsItem.setUrl(contentUrl);
            newsItem.setNewsImg(imageUrl);


            newsItem.setTitle(link.text());    //设置新闻标题

            Elements aboutItem = items.get(i).getElementsByClass("time");
            String date = aboutItem.text();
